...
export default (liveSocket: LiveSocket, eventType: string | null): AllJSCommands => {
  return {
    exec(el, encodedJS){
    ...
    show(el, opts = {}){
      ...
      JS.show(eventType, owner, el, opts.display, JS.transitionClasses(opts.transition), opts.time, opts.blocking)
    },
    hide(el, opts = {}){
      const owner = liveSocket.owner(el)
      JS.hide(eventType, owner, el, null, JS.transitionClasses(opts.transition), opts.time, opts.blocking)
    },
    toggle(el, opts = {}){
      const owner = liveSocket.owner(el)
      const inTransition = JS.transitionClasses(opts.in)
      const outTransition = JS.transitionClasses(opts.out)
      JS.toggle(eventType, owner, el, opts.display, inTransition, outTransition, opts.time, opts.blocking)
    },
    addClass(el, names, opts = {}){
      const classNames = Array.isArray(names) ? names : names.split(" ")
      const owner = liveSocket.owner(el)
      JS.addOrRemoveClasses(el, classNames, [], JS.transitionClasses(opts.transition), opts.time, owner, opts.blocking)
    },
    ...
    toggleClass(el, names, opts = {}){
      const classNames = Array.isArray(names) ? names : names.split(" ")
      const owner = liveSocket.owner(el)
      JS.toggleClasses(el, classNames, JS.transitionClasses(opts.transition), opts.time, owner, opts.blocking)
    },
