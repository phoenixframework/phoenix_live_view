        ...
        onNodeAdded: (el) => {
          ...
          if (el.nodeName === "SCRIPT" && el.hasAttribute(PHX_RUNTIME_HOOK)) {
            ...
            if (el.hasAttribute("nonce")) {
              ...
              nonce = template.content.querySelector(
...
var View = class _View {
  ...
  getFormsForRecovery() {
    ...
    ).map((form) => form.cloneNode(true)).reduce((acc, form) => {
      acc[form.id] = form;
      return acc;
    }, {});
  }
...
var LiveSocket = class {
  ...
  getHookCallbacks(name) {
    return name && name.startsWith("Phoenix.") ? hooks_default[name.split(".")[1]] || this.hooks[name] : this.hooks[name];
  }
  isUnloaded() {
