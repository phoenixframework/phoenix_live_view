defmodule Phoenix.LiveView.TagEngine do
  ...
  defp has_tags?([{:tag, _meta, _args} | _tokens]), do: true

  defp has_tags?([{:local_component, _meta, [_, _, args]} | tokens]) do
    children = List.wrap(Keyword.get(args, :do))
    has_tags?(children) or has_tags?(tokens)
  end

  defp has_tags?([{:remote_component, _meta, [_, _, args]} | tokens]) do
    children = List.wrap(Keyword.get(args, :do))
    has_tags?(children) or has_tags?(tokens)
  end
  ...
  def handle_end(state) do
    ...
    # we need to wrap the finalize call into an anonymous function
