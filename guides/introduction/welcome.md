...
Remember to replace the directory `my_app_web` and
the module `MyAppWeb` with your app's name:
...
Once the LiveView is rendered, a regular HTML response is sent.
When you generated your LiveView app with `mix`, it created
`./assets/js/app.js` with the following code:
