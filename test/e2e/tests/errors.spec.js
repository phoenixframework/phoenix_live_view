...
test.describe("exception handling", () => {
  ...
  test.describe("during connected mount", () => {
    ...
    test("reloads the page when connected mount fails", async ({ page }) => {
      ...
      await expect(networkEvents).toEqual(expect.arrayContaining([
        {
          "method": "GET",
     ...
     */
    test("abandons child remount if child LV fails multiple times", async ({ page }) => {
      ...
      await expect(consoleMessages.filter(m => m.startsWith("child "))).toEqual([
