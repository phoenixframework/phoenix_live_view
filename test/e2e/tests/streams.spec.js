...
test("stream reset removes items", async ({page}) => {
  ...
  expect(await usersInDom(page, "users")).toEqual([{id: "users-1", text: "chris"}, {id: "users-2", text: "callan"}])
...
test("stream reset updates attributes", async ({page}) => {
  ...
  await await expect(page.locator("#users-1")).toHaveAttribute("data-count", "0")
  await await expect(page.locator("#users-2")).toHaveAttribute("data-count", "0")
  ...
  await await expect(page.locator("#users-1")).toHaveAttribute("data-count", "1")
  await await expect(page.locator("#users-3")).toHaveAttribute("data-count", "1")
  await await expect(page.locator("#users-4")).toHaveAttribute("data-count", "1")
})
