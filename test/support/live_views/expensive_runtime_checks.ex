defmodule Phoenix.LiveViewTest.ExpensiveRuntimeChecksLive do
  ...
  @impl Phoenix.LiveView
  def handle_event("expensive_assign_async", _params, socket) do
    socket
    |> assign_async(:test, bad_assign_async_function(socket))
    |> then(&{:noreply, &1})
  end
  ...
  def handle_event("expensive_start_async", _params, socket) do
    socket
    |> start_async(:test, bad_start_async_function(socket))
    |> then(&{:noreply, &1})
  end
  ...
  defp bad_assign_async_function(socket) do
    fn ->
  ...
  defp bad_start_async_function(socket) do
