defmodule Phoenix.LiveView.LiveReloadTest do
  ...
  test "LiveComponents render again and keep their state when phoenix_live_reload is received" do
    %{conn: conn, socket: socket} = start(live_reload_config())

    Application.put_env(:phoenix_live_view, :vsn, 1)
    {:ok, lv, _html} = live(conn, "/live-reload")
    assert render(lv) =~ "Component version 1, count 0"

    assert lv |> element("#reload-component") |> render_click() =~
             "Component version 1, count 1"
    ...
    assert_receive {:phoenix_live_reload, :live_view, "lib/test_auth_web/live/user_live.ex"}
    assert render(lv) =~ "Component version 2, count 1"
  end
